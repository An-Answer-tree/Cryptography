#include <iostream>
#include <unordered_map> // create dictionary to change string to hex
#include <string.h>
#include <string>
...
void KeyExpansionCore(unsigned char* temp_word, unsigned char i)
{
	// rotate left
	...
	// s-box lookup
	temp_word[0] = s_box[temp_word[0]];
	temp_word[1] = s_box[temp_word[1]];
	temp_word[2] = s_box[temp_word[2]];
	temp_word[3] = s_box[temp_word[3]];
...
int main()
{
	...
	if(choose == 0)
	{
    	...
    	{
			// plaintxt need to encrypt every epoch
			unsigned char eachPlaintxt[16];
			for(int j = 0; j < 16; j++)
			{
