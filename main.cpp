...
void KeyExpansion(unsigned char* inputKey, unsigned char* expandedKey)
{
    ...
    unsigned char temp[4];    // temporary array for KeyExpansionCore

    while(bytesGenerated < 176)
    {
        // read 4 bytes for the KeyExpansionCore
        for(int i = 0; i < 4; i++)
        {
        ...
        for(int j = 0; j < 4; j++)
        {
            expandedKey[bytesGenerated] = expandedKey[bytesGenerated - 16] ^ temp[j];
            bytesGenerated++;
        }
    }
}
